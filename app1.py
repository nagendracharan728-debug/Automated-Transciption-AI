...
st.title("🎧 Podcast Topic & Sentiment Analyzer")
    ...
    try:
        ...
        with tab2:
            ...
            if st.session_state.search_index:
                ...
                st.warning("⚠️ Process podcast first to enable search")
        ...
        with tab3:
            ...
            # Segment selector with better naming
            segment_options = []
            ...
            # Segment info metrics with MM:SS format
            start_mmss = seconds_to_mmss(row['start_time'])
        ...
        # -----------------------------------
        # AUDIO PLAYER WITH JUMP TIMESTAMPS
        # -----------------------------------
        ...
        if st.session_state.audio_file:
            ...
            with col2:
                ...
                for idx, row in df.iterrows():
                    keywords_list = row.get("keywords", [])
                    if keywords_list and isinstance(keywords_list, list) and len(keywords_list) > 0:
                        segment_name = keywords_list[0][:20]  # Truncate long names
                    else:
                        segment_name = f"Segment {row['segment_id'] + 1}"
